...
\* Note: The subtle uppercase and lowercase differences were due to the assignment's particular instructions.<br>
...
source .env
mysql -h $DB_BOOKS_COMMANDS_URL -u $DB_ADMIN_USER -p"${DB_ADMIN_PASS}" \
  -e "CREATE DATABASE IF NOT EXISTS ${DB_BOOKS_COMMANDS_DATABASE};" \
  -e "CREATE USER ${DB_BOOKS_COMMANDS_USER} IDENTIFIED BY '${DB_BOOKS_COMMANDS_PASS}';" \
  -e "GRANT ALL PRIVILEGES ON ${DB_BOOKS_COMMANDS_DATABASE}.* TO ${DB_BOOKS_COMMANDS_USER};"

mysql -h $DB_CUSTOMERS_URL -u $DB_ADMIN_USER -p"${DB_ADMIN_PASS}" \
