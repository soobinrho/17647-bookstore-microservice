...
def check_circuit_breaker_open(session: Session) -> bool:
...
def check_should_circuit_breaker_close(session: Session) -> bool:
    when_circuit_breaker_open = session.get(Misc, MISC_KEY_WHEN_OPEN)
    when_open = int(when_circuit_breaker_open.misc_value)
    when_open = datetime.datetime.fromtimestamp(when_open)
    when_recheck = when_open + datetime.timedelta(seconds=CIRCUIT_BREAKER_WAIT_HOW_LONG)
...
def reset_circuit_breaker_time(session: Session) -> None:
    when_circuit_breaker_open = session.get(Misc, MISC_KEY_WHEN_OPEN)
...
def close_circuit_breaker(session: Session) -> None:
    when_circuit_breaker_open = session.get(Misc, MISC_KEY_WHEN_OPEN)
    session.delete(when_circuit_breaker_open)
    session.commit()


def open_circuit_breaker(session: Session) -> None:
